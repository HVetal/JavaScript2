...
class Cart {
  ...
  //       send(reject, resolve, `${API_URL}getBasket.json`)
  //     })
  //     .then((response) => {
  ...
  // fetchGoods() {
  //   send(this._onError, this._onSuccess.bind(this), `${API_URL}/cart`)
  // }
  // проверка наличия товара в корзине на сайте
  delGoods() {
    return new Promise((resolve, reject) => {
        send(reject, resolve, `${API_URL}/cart`)
      })
      .then((response) => {
...
class Showcase {
  ...
  fetchGoods() {
    ...
    // return new Promise((resolve, reject) => {
    //     send(reject, resolve, `${API_URL}/showcase`);
    //   })
    //   .then((response) => {
    //     this._onSuccess(response);
    //     this.filtered = this.list;
...
//const promiseCart = cart.fetchGoods();
promise.then(() => {
